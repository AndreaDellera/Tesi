\documentclass[a4paper]{article}
\usepackage[italian]{babel}
\usepackage[T1]{fontenc} % codifica dei font in uscita
\usepackage[utf8]{inputenc} % lettere accentate da tastiera 
\usepackage{mathptmx} % font in times new roman
\usepackage{cite} % pacchetto per le citazioni
\usepackage{harmony} % pacchetto per scrivere le note
\usepackage{amsmath} % pacchetto per scrivere le note
\newcommand{\setmetera}[2]{\ensuremath{\genfrac{}{}{0pt}{}{#1}{#2}}} % \setmetera{4}{4} per scrivere il tempo 4/4
\usepackage{natbib}

%grafici e plots
\usepackage{tikz}
\usepackage{pgfplots}
\pgfplotsset{compat=newest}
\usepgfplotslibrary{units}
\usetikzlibrary{matrix,chains,positioning,decorations.pathreplacing,arrows}

% impaginazione
\usepackage{geometry}
\geometry{margin=2cm}

%importazione di codice
\usepackage{listings}

\usepackage[font=small,labelfont=bf]{caption}
\usepackage{subcaption}
%scrittura di url
\usepackage{url}
%bibliografia
\begin{filecontents*}{biblio.bib}
@article{todd1989,
  title={A connectionist approach to algorithmic composition},
  author={Todd, Peter M},
  journal={Computer Music Journal},
  pages={27--43},
  year={1989},
  publisher={JSTOR}
}

@article{schaul2010pybrain,
  title={PyBrain},
  author={Schaul, Tom and Bayer, Justin and Wierstra, Daan and Sun, Yi and Felder, Martin and Sehnke, Frank and R{\"u}ckstie{\ss}, Thomas and Schmidhuber, J{\"u}rgen},
  journal={The Journal of Machine Learning Research},
  volume={11},
  pages={743--746},
  year={2010},
  publisher={JMLR. org}
}

@article{battiti1989accelerated,
  title={Accelerated backpropagation learning: Two optimization methods},
  author={Battiti, Roberto},
  journal={Complex systems},
  volume={3},
  number={4},
  pages={331--342},
  year={1989}
}

@inproceedings{kohavi1995study,
  title={A study of cross-validation and bootstrap for accuracy estimation and model selection},
  author={Kohavi, Ron and others},
  booktitle={Ijcai},
  volume={14},
  pages={1137--1145},
  year={1995}
}

@article{werbos1990backpropagation,
  title={Backpropagation through time: what it does and how to do it},
  author={Werbos, Paul J},
  journal={Proceedings of the IEEE},
  volume={78},
  number={10},
  pages={1550--1560},
  year={1990},
  publisher={IEEE}
}

@booklet{JohnsonRNN,
author = {Johnson, Daniel},
title = {Composing Music With Recurrent Neural Networks {@online}},
month = sep,
year = {2015},
url = {\url{http://www.hexahedria.com/2015/08/03/composing-music-with-recurrent-neural-networks/}}
}

\end{filecontents*}

% inizio del documento
\begin{document}

\input{copertina}
\input{dedica}
\newpage

\tableofcontents{Indice}
\newpage

\input{introduzione}
\newpage

\input{rnn}
\newpage

\input{allenamento}
\newpage

\input{codifica}
\newpage

\input{esperimenti}
\newpage

\input{conclusioni}
\newpage

\bibliographystyle{plain}
\bibliography{biblio}
\end{document}