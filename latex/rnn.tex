...
\subsection{RNN}
Una rete neurale ricorrente \`e una rete in cui esistono cammini che formano dei cicli. Il tipo di rete utilizzato in questa tesi \`e Long Short Term Memory (LSTM). Una rete LSTM ha la capacit\`a di ricordare gli elementi passati per un determinato tempo, quindi per un certo numero di attivazioni, ed \`e quindi indicata in problemi dove ci sono da imparare sequenze. Il grafo che la rappresenta \`e il seguente:
\input{graph_rnn}
\subsection{FFN}
Una rete neurale di questo tipo invece \`e la pi\`u classica, con cammini che vanno da uno strato al successivo senza formare cicli. Il grafo di questa rete \`e il seguente:
\input{graph_ffn}
...
Con pi\`u note si usano come input la probabilit\`a che la rete indovini il target aumenta perch\`e rendiamo esplicita un pezzo di storia della canzone (le $N$ note precedenti al target).
...
\end{itemize}
C'\`e una connessione piena tra il strato di input e lo strato nascosto, tra lo strato nascosto e il strato di output e tra lo strato nascosto e se stesso\footnote{I pesi che portano dal nodo $A$ al nodo $B$ possono essere diversi di quelli che portano dal nodo $B$ al nodo $A$;}.
La rete neurale non ricorrente \`e identica solo non presenta la connessione tra lo strato nascosto e se stesso come \`e gi\`a stato possibile vedere.\\
