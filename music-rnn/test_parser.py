...
def main():
    ...
    for j in range(len(codecs)):
        ...
        for i in range(0, len(codecs[j]) - 2, 1):
            ds[j].addSample((codecs[j][i][0], codecs[j][i][1], codecs[j][i][2], codecs[j][i][3], codecs[j][i][4],
                             codecs[j][i][5], codecs[j][i][6],
                             codecs[j][i][7], codecs[j][i][8], codecs[j][i][9], codecs[j][i][10])
                            ,
                            (codecs[j][i + 1][0], codecs[j][i + 1][1], codecs[j][i + 1][2], codecs[j][i + 1][3],
                             codecs[j][i + 1][4],
                             codecs[j][i + 1][5], codecs[j][i + 1][6], codecs[j][i + 1][7], codecs[j][i + 1][8],
                             codecs[j][i + 1][9],
                             codecs[j][i + 1][10])
                            )
    ...
    for j in range(len(tests)):
        ...
        for i in range(0, len(tests[j]) - 2, 1):
            dstest[j].addSample((tests[j][i][0], tests[j][i][1], tests[j][i][2], tests[j][i][3], tests[j][i][4],
                                 tests[j][i][5], tests[j][i][6],
                                 tests[j][i][7], tests[j][i][8], tests[j][i][9], tests[j][i][10]),
                                (tests[j][i + 1][0], tests[j][i + 1][1], tests[j][i + 1][2], tests[j][i + 1][3],
                                 tests[j][i + 1][4],
                                 tests[j][i + 1][5], tests[j][i + 1][6], tests[j][i + 1][7], tests[j][i + 1][8],
                                 tests[j][i + 1][9],
                                 tests[j][i + 1][10], )
                                )
    ...
    for i in range(len(ds)):
        x += trainer.trainOnDataset(ds[i], 100)
